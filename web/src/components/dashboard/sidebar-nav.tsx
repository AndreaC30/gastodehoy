...
export function Sidebar({
  ...
  function renderSidebar(mode: "desktop" | "mobile") {
    const navItems =
      mode === "mobile"
        ? ALL_NAV.filter((item) => !PRIMARY_IDS.includes(item.id))
        : ALL_NAV;
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          <div className="border-b border-[var(--color-border)] px-1.5 py-2">
            <p className="mb-1.5 px-2 font-mono text-[10px] font-medium uppercase tracking-[0.08em] text-[var(--color-text-dim)]">
              {mode === "mobile" ? t("nav.moreTitle") : t("nav.accountMenu")}
            </p>
            {mode === "mobile" ? (
              <p className="mb-2 px-2 text-[11px] leading-snug text-[var(--color-text-dim)]">
                {t("nav.tabsHint")}
              </p>
            ) : null}
              ...
              {navItems.map(({ id, labelKey, Icon }) => (
                <li key={id}>
                  <button
                    ...
                    className={`flex min-h-11 w-full items-center gap-3 rounded-lg border border-transparent px-3 py-2.5 text-left transition-colors ${
                      activeSection === id
              ...
              {(["es", "en", "fr", "de"] as const).map((lang) => {
                ...
                return (
                  <button
                    ...
                    className={`min-h-9 rounded-md px-2.5 py-1 text-[0.65rem] font-semibold uppercase tracking-wide transition-colors ${
                      active
        ...
        <div className="fixed inset-0 z-[60] md:hidden" role="presentation">
          ...
          <div className="relative flex h-full max-w-[min(100%,16rem)] justify-start shadow-[8px_0_24px_rgb(0_0_0_/_0.35)]">
            ...
            <button
              type="button"
              onClick={() => setMobileOpen(false)}
              className={`absolute right-2 top-3 z-10 inline-flex min-h-11 min-w-11 items-center justify-center rounded-lg border border-[var(--color-border)] bg-[var(--color-panel-elevated)] text-[var(--color-text-dim)] hover:border-[var(--color-accent-border)] hover:text-[var(--color-accent)] ${FOCUS_RING}`}
              aria-label={t("nav.closeMenu")}
            >
