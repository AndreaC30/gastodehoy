...
def list_expenses(
    ...
    stmt = (
        ...
        .where(VariableExpense.occurred_at >= start, VariableExpense.occurred_at <= end)
...
def create_expense(payload: VariableExpenseCreate, db: Session = Depends(get_db)) -> VariableExpense:
    day = payload.occurred_at or today_in_app_timezone()
    row = VariableExpense(amount=payload.amount, occurred_at=day, note=payload.note)
    db.add(row)
    db.commit()
    db.refresh(row)
    return row
